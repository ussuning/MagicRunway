...
public class CollectionEntry : MonoBehaviour {
    ...
    {
        m_MaxPosition = 0;
        m_MinPosition = -m_RectTransform.rect.height;
    }
    ...
    void Update()
    {
        if (m_AnimState == 1)
        {
            ...
            {
                m_RectTransform.anchoredPosition = Tween.SeptOut(m_CurrentPos, new Vector2(m_RectTransform.anchoredPosition.x,m_MaxPosition), m_AnimDeltaTime, m_AnimationDuration);
                m_reveal.color = Tween.Linear(m_reveal.color, new Color(255.0f, 255.0f, 255.0f, 0.0f), m_AnimDeltaTime, m_AnimationDuration);
            }
            else
            {
                m_RectTransform.anchoredPosition = new Vector2(m_RectTransform.anchoredPosition.x, m_MaxPosition);
                m_reveal.color = new Color(255.0f, 255.0f, 255.0f, 0.0f);
        ...
        if (m_AnimState == 2)
        {
            ...
            if (m_AnimDeltaTime <= m_AnimationDuration)
            {
                m_RectTransform.anchoredPosition = Tween.SeptIn(m_CurrentPos, new Vector2(m_RectTransform.anchoredPosition.x,m_MinPosition), m_AnimDeltaTime, m_AnimationDuration);
