...
public class InteractionManager : MonoBehaviour 
{
    ...
    protected Vector3 HandScreenPos;
    protected bool hitJoin = false;
...
//					monoScript.enabled)
				   ...
				   kinectManager.GetJointTrackingState(playerUserID, (int)KinectInterop.JointType.HandLeft) != KinectInterop.TrackingState.NotTracked)
				{
							...
							(allowPushToClick && leftHandPressProgress > 0f)))
					{
							...
							(allowPushToClick && leftHandPressProgress > 0.99f && isLeftHandPress))
						{
							if(!isLeftHandClick)
							{
                                //Debug.Log("Setting Cursor click progress to 1");
				   ...
				   kinectManager.GetJointTrackingState(playerUserID, (int)KinectInterop.JointType.HandRight) != KinectInterop.TrackingState.NotTracked)
				{
							...
							(allowPushToClick && rightHandPressProgress > 0f)))
					{
							...
							(allowPushToClick && rightHandPressProgress > 0.99f && isRightHandPress))
						{
							if(!isRightHandClick)
							{
                            //    Debug.Log("Right Hand, set cusor click progress to 1");
                ...
                */
				...
				if(isRightHandInteracting)
				{
                    ...
                    if (! joinInProgress)
                    {
                        ...
                        {
                            screenPos = Camera.current.ViewportToScreenPoint(rightHandScreenPos);    // convert viewport to screen space position
                            joinInProgress = true;
                            StartCoroutine(UpdateJoinLiveProgress(screenPos, rightHandClickProgress));
                        }
    ...
    IEnumerator UpdateJoinLiveProgress(Vector3 screenPos, float handClickProgress)
    {
        GraphicRaycaster m_Raycaster;
        PointerEventData m_PointerEventData;
        EventSystem m_EventSystem;

        m_Raycaster = uiStart.GetComponent<GraphicRaycaster>();
        m_EventSystem = uiStart.GetComponent<EventSystem>();
